...
export default class JtSelect extends HTMLElement {
    ...
    /** @private */
    _onFilterInput(ev: InputEvent) {
        if (ev.isComposing || !this.searchable) return;
        this._listbox.filter = (ev.target as HTMLInputElement).value;
    }
    ...
    _onItemClick(ev: CustomEvent) {
        if (!this.readOnly) {
            this._selectValue(ev.detail.value as string);
        }
        this._closeList();
    }
    ...
    constructor() {
        ...
        // Create Search Box
        this._filterInput = htmlToElement(`<input id="${this._id}-filter" type="search" tabindex="-1" placeholder="Search Options" />`) as HTMLInputElement;
        this._filterInput.addEventListener('input', (ev) => this._onFilterInput(ev as InputEvent));
        ...
        if (this._select instanceof HTMLElement) {
            ...
            if (this._select.id) {
                ...
                if (label instanceof HTMLLabelElement) {
                    ...
                    if (!label.id) label.id = `${this._id}-label`;
